...
namespace GubusAdminLTE.MyExtensions
{
    /// <summary>
    /// Spire için
    /// </summary>
    ...
    {
        /// <param name="TabloBossaBildir">Eğer tablo boşsa dokumandaki Text yerine «» yazar.</param>
        public static void mAddDataTable(this Document Doc, string Text, DataTable dt, bool TabloBossaBildir = true)
        {
            if (dt.Rows.Count > 0)
            {
                ...
                if (selection == null)
                {
                    Doc.Replace(Text, TabloBossaBildir ? "«»" : "", true, true);
                    return;
                }
                ...
                Table docTable = section.AddTable(true);
                docTable.ResetCells(dt.Rows.Count + 1, dt.Columns.Count);

                //Kolon başlıkları
                for (int col = 0; col < dt.Columns.Count; col++)
                {
                    docTable.Rows[0].Cells[col].AddParagraph().AppendText(dt.Columns[col].ColumnName);

                    docTable.Rows[0].Cells[col].CellFormat.VerticalAlignment = VerticalAlignment.Middle;
                    docTable.Rows[0].Cells[col].CellFormat.BackColor = Color.DarkGray;
                }

                for (int row = 0; row < dt.Rows.Count; row++)
                    for (int col = 0; col < dt.Columns.Count; col++)
                        docTable.Rows[row + 1].Cells[col].AddParagraph().AppendText(dt.Rows[row][col] as string);

                body.ChildObjects.Remove(paragraph);
                body.ChildObjects.Insert(index, docTable);
            }
            else
            {
                Doc.Replace(Text, TabloBossaBildir ? "«»" : "", true, true);
        ...
        /// <param name="Byte">Eklenecek olan image byte dizisi olarak verilmelidir.</param>
        public static void mAddImage(this Document Doc, string Text, byte[] Byte, bool BossaBildir = true)
        {
            ...
            if (selections == null)
            {
                Doc.Replace(Text, BossaBildir ? "«»" : "", true, true);
            ...
            //Traverse the document, remove the text content, insert the picture
